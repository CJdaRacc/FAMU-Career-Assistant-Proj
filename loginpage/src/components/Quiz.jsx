...
export default function Quiz({ user, onDone }) {
  ...
  const anyOtherSelected = Object.values(fieldSelections).some((arr) => (arr || []).includes('Other'))
  ...
  const handleSubmit = async (e) => {
    ...
    const preset = Object.values(fieldSelections)
      .flat()
      .filter((i) => i && i !== 'Other')
    const custom = (anyOtherSelected && otherText.trim())
    ...
    setSubmitting(true)
    try {
      const payload = { userId: user.userId, major: major.trim(), interests: finalInterests, classYear }
        ...
        <div className="col-12 col-lg-8">
          <div className="card shadow-sm">
            <div className="card-body p-4">
              <h2 className="mb-3" style={{ fontFamily: '"Limelight", serif' }}>Student Profile Setup</h2>
              <p className="text-muted">Tell us your major, general fields of study, related interests, and class year.</p>
              {error && (
                <div className="alert alert-danger py-2" role="alert">{error}</div>
              )}
              <form onSubmit={handleSubmit}>
                <div className="mb-3">
                  <label className="form-label fw-semibold" htmlFor="major">Major</label>
                  <input
                    id="major"
                    type="text"
                ...
                <div className="mb-3">
                  <label className="form-label fw-semibold">General Fields of Study</label>
                  <div className="row">
                    {fieldNames.map((field) => (
                      <div className="col-12 col-md-6" key={field}>
                        <div className="form-check mb-2">
                          ...
                          />
                          <label className="form-check-label" htmlFor={`field-${field}`}>{field}</label>
                        </div>
                      </div>
                    ))}
                  </div>
                  <div className="form-text">Select one or more fields. Interest options will appear below for each selected field.</div>
                </div>

                {/* Per-field interests dropdowns */}
                {selectedFields.map((field) => (
                  <div className="mb-3" key={`sel-${field}`}>
                    <label className="form-label fw-semibold" htmlFor={`interests-${field}`}>{field} Interests</label>
                    <select
                      id={`interests-${field}`}
                    ...
                    >
                      {FIELD_INTERESTS[field].map((opt) => (
                        <option key={`${field}-${opt}`} value={opt}>{opt}</option>
                      ))}
                    </select>
                    <div className="form-text">Hold Ctrl (Windows) or Command (Mac) to select multiple. Choose "Other" to add custom interests.</div>
                  </div>
                ))}

                {anyOtherSelected && (
                  <div className="mb-3">
                    <label className="form-label" htmlFor="otherInterests">Other interests (comma-separated)</label>
                    <input
                      id="otherInterests"
                      type="text"
                      className="form-control"
                ...
                <div className="mb-3">
                  <label className="form-label fw-semibold" htmlFor="classYear">Class Year</label>
                  <select
                    id="classYear"
                    className="form-select"
                    value={classYear}
