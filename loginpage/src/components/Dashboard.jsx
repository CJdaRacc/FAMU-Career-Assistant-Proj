...
function useSavedApplied() {
  ...
  useEffect(() => { writeLs(SAVED_KEY, saved) }, [saved])
  useEffect(() => { writeLs(APPLIED_KEY, applied) }, [applied])
  ...
  const applyJob = (job) => {
    setApplied((prev) => prev.some(j => j.id === job.id) ? prev : [...prev, job])
          ...
          <div key={job.id} className="job-card job-card-orange shadow-sm">
            <div className="d-flex align-items-center">
              ...
              <div className="flex-grow-1">
                ...
                <div className="job-sub text-white-50 small">{job.company} • {job.location} • {job.type}</div>
                <div className="job-skills mt-2 small">
                  <div><strong>Matched:</strong> {job.matchedSkills.join(', ')}</div>
                  <div><strong>Missing:</strong> {job.missingSkills.join(', ')}</div>
                </div>
              </div>
              <div className="d-flex flex-column flex-sm-row gap-2 ms-3">
                {isSaved ? (
                  <button className="btn btn-light btn-sm" onClick={() => unsaveJob(job.id)}>Unsave</button>
                ) : (
                  <button className="btn btn-outline-light btn-sm" onClick={() => saveJob(job)}>Save</button>
                )}
                <button
